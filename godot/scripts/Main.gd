...
func _create_entity_snapshot(entity: Dictionary) -> EntitySnapshot:
	...
	return EntitySnapshot.from_dictionary({
		"id": str(entity.get("id", "")), "name": str(entity.get("name", "")),
		"type": str(entity.get("type", "NPC")), "x": int(position.get("x", 0)),
		"y": int(position.get("y", 0)), "hp": int(entity.get("hp", 0)),
		"maxHp": int(entity.get("maxHp", 0)), "armorClass": int(entity.get("armorClass", 10)),
...
func _cleanup_removed_entities(active_ids: Dictionary[String, bool]) -> void:
	for entity_id in entity_nodes.keys():
		if entity_id == PLAYER_ID or active_ids.has(entity_id):
			continue
