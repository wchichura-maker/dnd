...
## Terminal state: DEATH, which freezes on its final frame.
...
func _on_animation_finished() -> void:
	...
	if current_state == CharacterAnimationState.State.DEATH and death_locked:
		var death_animation := _animation_name(CharacterAnimationState.State.DEATH)
		var death_frames := animated_sprite.sprite_frames.get_frame_count(death_animation)
		if death_frames > 0:
