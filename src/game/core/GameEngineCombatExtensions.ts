...
export class GameEngineCombatExtensions extends GameEngine {
  ...
  ensureAutomaticCombat(): ActionResult | null {
    ...
    for (let i = 0; i < living.length; i++) {
      for (let j = i + 1; j < living.length; j++) {
        ...
        if (result.success) this.setState({ ...this.getState(), logs: [...this.getState().logs, `${a.name} e ${b.name} entraram em combate automaticamente.`] });
        return result;
      }
    }
    return null;
  ...
  override endTurn(): ActionResult {
    ...
    if (result.success) {
      ...
      if (this.chargeAcPenaltyActive.has(id)) {
        ...
        if (entity) this.setState({ ...this.getState(), entities: this.getState().entities.map(item => item.id === id ? { ...item, dnd: { ...item.dnd, defense: { ...item.dnd.defense, miscBonus: item.dnd.defense.miscBonus + 2 } } : item), logs: [...this.getState().logs, `${entity.name}: penalidade de CA da investida terminou.`] });
  ...
  override executeAction(action: GameAction): ActionResult {
    ...
    if (action.type === "ATTACK" && result.success && this.shouldEndCombatAfterDeath()) {
      ...
      return { ...result, message: `${result.message} ${combatEnd.message}`, data: { ...(result.data ?? {}), combatEnded: true, combatEndReason: "DEATH" } };
    }
    return result;
  }
  ...
  private executeWithdraw(action: GameAction): ActionResult {
    ...
    const steps = path.path.length && path.path[0].x === actor.position.x && path.path[0].y === actor.position.y ? path.path : [actor.position, ...path.path];
    ...
    if (opportunity) { this.setState({ ...this.getState(), turn: originalTurn }); return opportunity; }
    const move = super.executeAction({ type: "MOVE", actorId: actor.id, destination: action.destination });
    if (!move.success) { this.setState({ ...this.getState(), turn: originalTurn }); return { success: false, message: `Retirada falhou: ${move.message}` }; }
    const after = consumeFullRoundAction(this.getState().turn);
    this.setState({ ...this.getState(), turn: after, logs: [...this.getState().logs, `${actor.name} realizou RETIRADA e moveu ${path.cost} casas.`] });
  ...
  private executeRun(action: GameAction): ActionResult {
    ...
    if (opportunity) { this.setState({ ...this.getState(), turn: originalTurn }); return opportunity; }
    const move = super.executeAction({ type: "MOVE", actorId: actor.id, destination: action.destination });
    if (!move.success) { this.setState({ ...this.getState(), turn: originalTurn }); return { success: false, message: `Corrida falhou: ${move.message}` }; }
    const after = consumeFullRoundAction(this.getState().turn);
    this.setState({ ...this.getState(), turn: after, logs: [...this.getState().logs, `${actor.name} realizou CORRIDA e moveu ${path.cost} casas.`] });
  ...
  private resolveOpportunityAttacksForPath(actorId: string, steps: Array<{ x: number; y: number }>, allowedTransitions?: Array<{ from: { x: number; y: number }; to: { x: number; y: number }; index: number }>): ActionResult | null {
    ...
    for (const defender of state.entities) {
      ...
      if (!actor || !canAct(actor)) return { success: false, message: `${state.entities.find(entity => entity.id === actorId)?.name ?? actorId} não pode concluir o movimento após o ataque de oportunidade.`, data: { opportunityAttacks } };
      ...
      opportunityAttacks.push({ attackerId: defender.id, targetId: actor.id, roll: attackResult.roll, attackBonus: attackResult.attackBonus, total: attackResult.total, critical: attackResult.critical, hit: attackResult.hit, damage: attackResult.damage, hpBefore: actor.hp, hpAfter: newHp });
      this.setState({ ...this.getState(), entities: this.getState().entities.map(entity => entity.id === actor.id ? updatedActor : entity), logs: [...this.getState().logs, `${defender.name} realizou um ATAQUE DE OPORTUNIDADE contra ${actor.name}.`, `D20: ${attackResult.roll} + ${attackResult.attackBonus} = ${attackResult.total}.`, `CA de ${actor.name}: ${getArmorClass(actor)}.`, attackResult.critical ? "CRÍTICO!" : "", attackResult.hit ? `ACERTO. Dano: ${attackResult.damage}. ${actor.name}: ${actor.hp} → ${newHp} HP.` : "ERRO.", `Estado de ${actor.name}: ${getHitPointState(updatedActor)}.`].filter(Boolean) });
      ...
      if (!canAct(updatedActor)) return { success: false, message: `${updatedActor.name} ficou ${getHitPointState(updatedActor)} após o ataque de oportunidade e não pode concluir o movimento.`, data: { opportunityAttacks, targetDied: false, targetState: getHitPointState(updatedActor) } };
    }
    return null;
  }
  ...
  private executeCharge(action: GameAction): ActionResult {
    ...
    if (opportunity) { this.setState({ ...this.getState(), turn: originalTurn }); return opportunity; }
    const move = super.executeAction({ type: "MOVE", actorId: actor.id, destination: action.destination });
    if (!move.success) { this.setState({ ...this.getState(), turn: originalTurn }); return { success: false, message: `Investida falhou: ${move.message}` }; }
    ...
    const chargedActor = { ...movedActor, dnd: { ...movedActor.dnd, defense: { ...movedActor.dnd.defense, miscBonus: movedActor.dnd.defense.miscBonus - 2 } } };
    ...
    if (isDead(targetAfterAttack)) {
      const combatEnded = this.shouldEndCombatAfterDeath();
      if (combatEnded) this.resolveCombat("DEATH");
      return { success: true, message: combatEnded ? `${movedTarget.name} morreu pela investida. ${getCombatEndMessage("DEATH")}` : `${movedTarget.name} morreu pela investida.`, data: { charge: true, chargeAttackBonus: 2, chargeAcPenalty: -2, position: action.destination, distance: path.cost, damage: attackResult.damage, roll: attackResult.roll, attackBonus: attackResult.attackBonus, total: attackResult.total, targetDied: true, combatEnded, combatEndReason: combatEnded ? "DEATH" : undefined } };
    }
    return { success: true, message: `${chargedActor.name} realizou uma INVESTIDA contra ${movedTarget.name}. ${attackResult.hit ? `Acerto por ${attackResult.total} contra CA ${attackResult.targetArmorClass}, causando ${attackResult.damage} de dano.` : `Errou o ataque (${attackResult.total} contra CA ${attackResult.targetArmorClass}).`}`, data: { charge: true, chargeAttackBonus: 2, chargeAcPenalty: -2, position: action.destination, distance: path.cost, damage: attackResult.damage, roll: attackResult.roll, attackBonus: attackResult.attackBonus, total: attackResult.total, targetId: movedTarget.id } };
  }

  private shouldEndCombatAfterDeath(): boolean {
    const state = this.getState();
    const living = state.entities.filter(entity => !isDead(entity));
    for (let i = 0; i < living.length; i++) for (let j = i + 1; j < living.length; j++) if (this.isHostile(state, living[i].id, living[j].id)) return false;
    return true;
  }

  private isExplorationMode(): boolean { return this.getState().mode === "EXPLORATION"; }
  private getActiveEntity(): Combatant | undefined { return this.getState().entities.find(entity => entity.id === this.getState().turn.characterId); }
  private isHostile(state: ReturnType<GameEngine["getState"]>, a: string, b: string): boolean { return state.relationships.some(relationship => ((relationship.entityAId === a && relationship.entityBId === b) || (relationship.entityAId === b && relationship.entityBId === a)) && relationship.hostile); }
  private distance(a: { x: number; y: number }, b: { x: number; y: number }): number { const dx = Math.abs(b.x - a.x); const dy = Math.abs(b.y - a.y); const diagonal = Math.min(dx, dy); const straight = Math.max(dx, dy) - diagonal; return Math.floor(diagonal / 2) * 3 + (diagonal % 2) + straight; }
  ...
  private executeCoupDeGrace(action: GameAction): ActionResult {
    ...
    this.setState({ ...this.getState(), entities: this.getState().entities.map(entity => entity.id === finalTarget.id ? finalTarget : entity), turn: after, logs: [...this.getState().logs, `${actor.name} realizou um GOLPE DE MISERICÓRDIA contra ${target.name}.`, `Dano: ${damage}.`, `Fortitude: ${fortitudeRoll} + ${fortitudeBonus} = ${fortitudeTotal}. CD: ${10 + damage}.`, resolved.message] });
    if (resolved.targetDied || isDead(finalTarget)) {
      const combatEnded = this.shouldEndCombatAfterDeath();
      if (combatEnded) this.resolveCombat("DEATH");
      return { success: true, message: combatEnded ? `${target.name} morreu pelo golpe de misericórdia. ${getCombatEndMessage("DEATH")}` : `${target.name} morreu pelo golpe de misericórdia.`, data: { damageRoll, fortitudeRoll, fortitude: fortitudeTotal, damage, targetDied: true, combatEnded, combatEndReason: combatEnded ? "DEATH" : undefined } };
    }
    return { success: true, message: `${actor.name} realizou um GOLPE DE MISERICÓRDIA.`, data: { damageRoll, fortitudeRoll, fortitude: fortitudeTotal, damage, targetDied: false } };
  }
}
